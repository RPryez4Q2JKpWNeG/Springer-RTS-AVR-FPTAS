...
from bpckp_fxns import apx_exact_24
...
class ExperimentRunner:
    ...
    def create_base_file_name_from_exp(self,exp,exp_desc):
        ...
        n_cores = exp.algorithm_params.n_cores
        parallelization_type = exp.algorithm_params.parallelization_type

        precision_str = str(precision)
        if precision <= 9:
            precision_str = "0"+str(precision)

        cores_str = str(int(n_cores))
        if int(n_cores) <= 9:
            cores_str = "0"+str(int(n_cores))
        ...
        csv_name_arr = [
            ...
            task_set_desc,
            delta_set_desc,
            "m"+str(int(memoization)),
            "s"+str(int(give_sln_seq)),
            "t"+str(int(trace)),
            "c"+cores_str,
            "p"+str(int(parallelization_type))
        ]
    ...
    def run_experiments(self,exp_list,make_dbf_files,make_exp_files,make_agg_file,exp_desc,verbose,use_timestamps=USE_TIMESTAMPS_DEFAULT):
        ...
        if make_agg_file:
            ...
            "e_r","e_b","e_f","1-E","Sln Multiplier","MEMO","SLN_SEQ","TRACE","CORES","PARA_TYPE","Log time(YYYY-MM-DD-HH-MN-SS)"]
        ...
        if make_exp_files:
            ...
            exp_file_headers = ["Runtime (s)","M(#)","W(RPM)","C(us)","A(RPM^2)",
        ...
        for exp_index in range(num_exps):
            ...
            self.run_experiment(exp_list[exp_index],dbf_file_csv_handle,exp_file_csv_handle,agg_file_csv_handle,verbose)
    ...
    def run_experiment(self,exp,
        ...
        alg = alg.lower()
        assert (alg == "apx" or alg == "exact" or alg == "row17" or alg == "kavr24" or alg == "kavr18")

        max_mem_usage = -1
        delta_table = [[-1]]
        experiment_total_runtime_s = -1

        if alg == "apx": #Approximate
            bpckp_instance = apx_exact_24.ApxExact24(task_set, apx_instance,n_cores,parallelization_type)
            delta_table = [[]]
            if trace:
                tracemalloc.start()
            (delta_table,experiment_total_runtime_s) = bpckp_instance.calculate_demand_seq(verbose, delta_set, True, True)
            if trace:
                mem_usage = tracemalloc.get_traced_memory()
                tracemalloc.stop()
                max_mem_usage =  max(mem_usage)

        elif alg == "exact": #Exact
            bpckp_instance = apx_exact_24.ApxExact24(task_set, apx_instance,n_cores,parallelization_type)
            delta_table = [[]]
            if trace:
                tracemalloc.start()
            (delta_table,experiment_total_runtime_s) = bpckp_instance.calculate_demand_seq(verbose, delta_set, True, False)
            if trace:
                mem_usage = tracemalloc.get_traced_memory()
                tracemalloc.stop()
                max_mem_usage =  max(mem_usage)

        elif alg == "kavr24": #2024 KAVR
            kavr_instance = KAVR_24.Kavr2024(task_set,precision,memoization,give_sln_seq,verbose,n_cores,parallelization_type)
            delta_table = [[]]
            if trace:
                tracemalloc.start()
            (delta_table,experiment_total_runtime_s) = kavr_instance.calculate_exact_demand(delta_set)
            if trace:
                mem_usage = tracemalloc.get_traced_memory()
                tracemalloc.stop()
                max_mem_usage =  max(mem_usage)

        elif alg == "kavr18": #2018 KAVR
            kavr_instance = KAVR_18.Kavr2018(task_set,precision,memoization,give_sln_seq,verbose)
            delta_table = [[]]
            if trace:
                tracemalloc.start()
            (delta_table,experiment_total_runtime_s) = kavr_instance.calculate_exact_demand(delta_set)
            ...
            row_instance = ROW_17.Row2017(task_set,precision,memoization,give_sln_seq,verbose,n_cores,parallelization_type)
            delta_table = [[]]
            if trace:
                tracemalloc.start()
            (delta_table,experiment_total_runtime_s) = row_instance.calculate_exact_demand(delta_set)
            if trace:
                mem_usage = tracemalloc.get_traced_memory()
                tracemalloc.stop()
                max_mem_usage =  max(mem_usage)
            ...
            print("Error: No correct algorithm selected")
            assert(0)
        ...
        if exp_file_csv_handle != -1:
            ...
            exp_csv_row_data = [experiment_total_runtime_s,
        ...
        if agg_file_csv_handle != -1:
            ...
            for i in range(len(delta_table)):
                ...
                agg_csv_row_data = [delta_us,demand_us,runtime_s,c_runtime_s,
                    ...
                    apx_instance.epsilon_r,apx_instance.epsilon_b,
                    apx_instance.epsilon_f,apx_instance.one_minus_epsilon,
                    apx_instance.solution_multiplier,memoization,give_sln_seq,trace,n_cores,parallelization_type]
